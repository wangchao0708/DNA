...
# 2. encode images and prompts with each candidate SD2 model,
...
SOURCE_FOLDERS = ['SD2-base-resize', 'SD2.1-base-resize', 'SD2-typography-resize', 'SD2-cartoon-resize']
MODEL_IDS = {
    'SD2-base':       'sd2-community/stable-diffusion-2-base',
    'SD2.1-base':     'sd2-community/stable-diffusion-2-1-base',
    'SD2-typography': 'pbevan11/stable-diffusion-2-typography',
    'SD2-cartoon': 'Norod78/sd2-cartoon-blip'
}
NUM_IMAGES = None
NUM_NOISE = None
TIMESTEPS = []
...
RESULTS_CSV = 'DNA_SD2_results.csv'
SUMMARY_JSON = 'DNA_SD2_summary.json'
BLIP_CACHE = 'DNA_SD2_blip_captions.json'
...
def encode_prompts(tokenizer, text_encoder, prompts, device, dtype, batch_size=32, max_length=77):
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        emb = text_encoder(tokens.input_ids.to(device))[0]  # [B, 77, 1024] for SD2, kept on GPU
        all_embs.append(emb)
        del tokens
    return torch.cat(all_embs, dim=0)  # [N, 77, 1024], GPU
...
def main():
    ...
    header = f"{'':>10}" + "".join(f"{m:>16}" for m in SOURCE_FOLDERS)
    print(f"  {header}")
    for s in SOURCE_FOLDERS:
        row = f"{s:>10}" + "".join(f"{confusion_avg[s][m]:>16}" for m in SOURCE_FOLDERS)
        print(f"  {row}")
